...
WHERE GV.MAGV IN (
					SELECT GV.MAGV
					FROM GIAOVIEN GV
					LEFT JOIN GIAOVIEN BM ON BM.MABM=GV.MABM
					GROUP BY GV.MAGV
...
WHERE GV.MAGV IN (
    SELECT GV1.MAGV
    FROM GIAOVIEN GV1
    LEFT JOIN DETAI DT1 ON DT1.GVCNDT=GV1.MAGV
    WHERE GV1.MAGV <> GV.MAGV
    GROUP BY GV1.MAGV
    HAVING COUNT(DISTINCT DT1.MADT) >= ALL (
        SELECT COUNT(DISTINCT DT2.MADT)
        FROM GIAOVIEN GV2
        LEFT JOIN DETAI DT2 ON DT2.GVCNDT=GV2.MAGV

...
WHERE NOT EXISTS (
    ...
    AND NOT EXISTS (
        ...
        FROM THAMGIADT TG
        WHERE TG.MADT=DT.MADT
        AND TG.MAGV = GV.MAGV -- Correlation with outer query (DETAI)
    )
)
...
WHERE TG.MAGV IN (SELECT GV.MAGV
					...
					WHERE BM.TENBM=N'HỆ THỐNG THÔNG TIN' )
GROUP BY DT.TENDT
HAVING COUNT(DISTINCT TG.MAGV)= (SELECT COUNT(DISTINCT GV.MAGV)
					FROM GIAOVIEN GV
					LEFT JOIN BOMON BM ON BM.MABM=GV.MABM
					WHERE BM.TENBM=N'HỆ THỐNG THÔNG TIN' )
...
JOIN BOMON BM ON GV.MABM = BM.MABM
WHERE BM.TENBM = N'HÓA HỮU CƠ'
GROUP BY DT.MADT, DT.TENDT, BM.MABM
...
--Q64. Cho biết tên giáo viên nào mà tham gia tất cả các công việc của đề tài 006.
 SELECT GV.HOTEN
 FROM GIAOVIEN GV
 WHERE NOT EXISTS (SELECT CV.STT, CV.MADT
					FROM CONGVIEC CV
					WHERE CV.MADT='006'
					EXCEPT
 ...
 WHERE NOT EXISTS (SELECT CV.STT, CV.MADT
					...
					SELECT TG.STT, TG.MADT
					FROM THAMGIADT TG
					WHERE TG.MAGV=GV.MAGV AND TG.MADT='006' AND CV.STT=TG.STT AND CV.MADT=TG.MADT)
					)
...
WHERE DT.TENDT=N'NGHIÊN CỨU TẾ BÀO GỐC'
GROUP BY GV.MAGV, GV.HOTEN
HAVING COUNT(TG.STT) = (SELECT COUNT(DISTINCT CV.STT)
					FROM CONGVIEC CV
					JOIN DETAI DT ON DT.MADT=CV.MADT
					WHERE DT.TENDT=N'NGHIÊN CỨU TẾ BÀO GỐC'
					)
...
SELECT GV.MAGV, GV.HOTEN, BM.TENBM, QL.HOTEN AS TENQL
FROM GIAOVIEN GV
JOIN BOMON BM ON BM.MABM = GV.MABM
JOIN THAMGIADT TG ON TG.MAGV = GV.MAGV
JOIN DETAI DT ON DT.MADT = TG.MADT
